...
def train_SVM_poly(DTR, LTR, C, c = 1 , d = 2 , p = 0, K = 1):
    
    Z = numpy.zeros(LTR.shape)
    Z[LTR == 1] = 1
    Z[LTR == 0] = -1
    
    poly = (numpy.dot(DTR.T, DTR)+c)**d+ K**2
    
    H = mcol(Z) * mrow(Z) * poly
...
def quad_svm(D,L,DE,LE,C):
    
    wStar = train_SVM_poly(D, L, C)
    ker = (numpy.dot(D.T, DE)+1)**2
    scores = numpy.dot(wStar.T, ker).sum(0).tolist()
...
def k_fold(D,L,k, gaussianize=0,seed=0):
    ...
    for pi in priors:
        ...
        for i in range(k):
            ...
            wStar = train_SVM_poly(DTR, LTR, 1e-1)
            ker = (numpy.dot(DTR.T, DTE)+1)**2
            scores.extend(numpy.dot(wStar.T, ker).sum(0).tolist())
        ...
        #else :
        print('minDCF SVM with C ', 0.1 ,' and application ', pi,', ', 1,', ', 1 , ' : ', "%.3f" % DCF.compute_min_DCF(scores, numpy.hstack(labels), pi, 1, 1))
    ...
    for pi in priors:
        ...
        for c in C:
            ...
            for i in range(k):
                ...
                #DTE=PCA(P,DTE)
                labels.append(LTE.tolist())
                wStar = train_SVM_poly(DTR, LTR, c)
                ker = (numpy.dot(DTR.T, DTE)+1)**2
                scores.extend(numpy.dot(wStar.T, ker).sum(0).tolist())
                #scores.extend(trainLinearSVM(DTR, LTR, 1, 1e-3, DTE))
                idx = numpy.roll(idx,nTest,axis=0)
        ...
        print(y)
    DCF.plot_DCF(C, y, 'C', 'Poly_SVM_tuning_PCA7.svg')
            #if gaussianized == 1:
                #print('minDCF LR with prior ', pi_T ,' and application ', pi,', ', 1,', ', 1 , ' with gaussianized features : ', DCF.compute_min_DCF(scores, numpy.hstack(labels), pi, 1, 1))
            #else :
