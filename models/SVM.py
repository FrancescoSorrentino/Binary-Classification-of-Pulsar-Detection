...
def train_SVM_linear(DTR, LTR, C, p = 0, K = 1):
    #DTREXT ITS NOT NEEDED IN THE KERNEL COMPUTATION!
    ...
    ###COMMENT THE LINE BELOW IF YOU USE EXP KERNEL
    H = numpy.dot(DTREXT.T, DTREXT)
    H = mcol(Z) * mrow(Z) * H
    alphaStar = compute_opt(H,DTR,LTR,C,p)
 
    return numpy.dot(DTREXT, mcol(alphaStar) * mcol(Z))
...
def compute_opt(H, DTR, LTR, C, p):
    ...
    alphaStar, _x, _y = scipy.optimize.fmin_l_bfgs_b( LDual, numpy.zeros(DTR.shape[1]), bounds = [(0,C)] * DTR.shape[1], iprint = 1,factr = 1.0 ,maxiter = 100000,
        maxfun=100000,)
...
def linear_svm(D,L,DE,LE,C):
    wStar = train_SVM_linear(D, L, C)
    DEX = numpy.vstack([DE,numpy.ones((1,DE.shape[1]))])
    scores = numpy.dot(wStar.T, DEX).sum(0).tolist()
...
def k_fold(D,L,k, gaussianize=0,seed=0):
    ...
    for i in range(k):
        ...
        DTEX = numpy.vstack([DTE,numpy.ones((1,DTE.shape[1]))])
        wStar = train_SVM_linear(DTR, LTR, 1e-1)
        scores.extend(numpy.dot(wStar.T, DTEX).sum(0).tolist())
...
def CEstimation(D,L,k, gaussianize=0,seed=0):
    nTrain = int(D.shape[1]*(k-1)/k)
    nTest = int(D.shape[1]) - nTrain
    numpy.random.seed(seed) 
    ...
    for pi in priors:
        ...
        for c in C:
            ...
            for i in range(k):
                ...
                labels.append(LTE.tolist())
                DTEX = numpy.vstack([DTE,numpy.ones((1,DTE.shape[1]))])
                wStar = train_SVM_linear(DTR, LTR, c)
                scores.extend(numpy.dot(wStar.T, DTEX).sum(0).tolist())
                #scores.extend(trainLinearSVM(DTR, LTR, 1, 1e-3, DTE))
                idx = numpy.roll(idx,nTest,axis=0)
            print('C: ', c)
        ...
        print(y)
    DCF.plot_DCF(C, y, 'C', 'L_SVM_CEst_PCA7.svg')
            #if gaussianized == 1:
                #print('minDCF LR with prior ', pi_T ,' and application ', pi,', ', 1,', ', 1 , ' with gaussianized features : ', DCF.compute_min_DCF(scores, numpy.hstack(labels), pi, 1, 1))
            #else :
            #print('minDCF SVM with C ', 1.0 ,' and application ', pi,', ', 1,', ', 1 , ' : ', "%.3f" % DCF.compute_min_DCF(scores, numpy.hstack(labels), pi, 1, 1))
