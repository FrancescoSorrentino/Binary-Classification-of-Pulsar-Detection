...
def GMM_EM(X,gmm,psi= 0.01): #X -> ev
    llNew=None
    llOld=None
    G=len(gmm)
    N=X.shape[1]
...
def k_fold(D,L,k, alpha, G, gaussianize=0,seed=0):
    ...
    for i in range(k):
        ...
        LTR = L[idxTrain] 
        LTE = L[idxTest]
        scores.extend(GMM_Full(DTR,DTE,LTR,alpha,2**G,'TiedDiag').tolist())
        labels.append(LTE.tolist())
        idx = numpy.roll(idx,nTest,axis=0)
        
    for pi in priors:
        print('minDCF GMM with application Diag-Cov + Z-score + 64c', pi,', ', 1,', ', 1 , ' : ', "%.3f" % DCF.compute_min_DCF(scores, numpy.hstack(labels), pi, 1, 1))
...
def k_fold05(D,L,k, alpha, G, gaussianize=0,seed=0):
    nTrain = int(D.shape[1]*(k-1)/k)
    nTest = int(D.shape[1]) - nTrain
    numpy.random.seed(seed) 
    ...
    #else :
    minDCFDiag05=DCF.compute_min_DCF(scoresDiag, numpy.hstack(labels), 0.5, 1, 1)
    minDCFTied05=DCF.compute_min_DCF(scoresTied, numpy.hstack(labels), 0.5, 1, 1)
    minDCFFull05=DCF.compute_min_DCF(scoresFull, numpy.hstack(labels), 0.5, 1, 1)
    minDCFTiedDiag05=DCF.compute_min_DCF(scoresTiedDiag, numpy.hstack(labels), 0.5, 1, 1)
...
def tuning_components(ZD,GD,L):
    ...
    # ----------------
    componentsToTry=[1,2,3,4,5,6]
    for i in componentsToTry:
        print("components: ",i)
        matriceDiagZ, matriceTiedZ, matriceFullZ, matriceTiedDiagZ = k_fold05(ZD,L,3,0.1,i)
        matriceDiagG, matriceTiedG, matriceFullG, matriceTiedDiagG = k_fold05(GD,L,3,0.1,i)
        # Z NORM
    ...
    # ----------- PLOTS -----------------------
    DCF.plot_DCF_GMM(componentsToTry, numpy.hstack(vettoreMatriciDiagZ), numpy.hstack(vettoreMatriciDiagG), "GMM Diagonal")
    DCF.plot_DCF_GMM(componentsToTry, numpy.hstack(vettoreMatriciTiedZ), numpy.hstack(vettoreMatriciTiedG), "GMM_Tied")
    DCF.plot_DCF_GMM(componentsToTry, numpy.hstack(vettoreMatriciFullZ), numpy.hstack(vettoreMatriciFullG), "GMM_Full")
    DCF.plot_DCF_GMM(componentsToTry, numpy.hstack(vettoreMatriciTiedDiagZ), numpy.hstack(vettoreMatriciTiedDiagG), "GMM_TiedDiag")
